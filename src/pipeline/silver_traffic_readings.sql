...
-- MAGIC %md
-- MAGIC # Silver: Traffic Readings
-- MAGIC
-- MAGIC Cleans and enriches synthetic traffic data:
-- MAGIC - Deduplicates by corridor_id + polled_at
-- MAGIC - Classifies congestion severity (free_flow → severe)
-- MAGIC - Adds time dimensions (hour, day_of_week, rush_hour flag)

-- COMMAND ----------

CREATE OR REFRESH MATERIALIZED VIEW silver_traffic_readings
COMMENT 'Cleaned traffic readings with congestion classification'
...
WITH deduplicated AS (
  ...
  WHERE api_response_json NOT LIKE '%"error"%'

...
-- MAGIC
-- MAGIC The headline table — joins GTFS scheduled travel times with
-- MAGIC traffic congestion data to show where transit is running
...
WITH route_scheduled AS (
  ...
  WHERE scheduled_segment_seconds > 0
    AND scheduled_segment_seconds < 7200
...
SELECT
  ...
  ROUND(
    (aa.actual_seconds - rs.avg_scheduled_seconds) / NULLIF(rs.avg_scheduled_seconds, 0) * 100, 1
  ) AS delay_pct

FROM route_scheduled rs
CROSS JOIN api_actual aa
WHERE rs.departure_hour = aa.hour_of_day
  AND aa.actual_seconds IS NOT NULL;
